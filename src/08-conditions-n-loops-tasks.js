...
function isInsideCircle(circle, point) {
  const distPoint = (point.x - circle.center.x) * (point.x - circle.center.x)
...
function getDigitalRoot(num) {
  while (num > 9) {
    const res = num.toString().split('').reduce((acc, curr) => Number(acc) + Number(curr));
